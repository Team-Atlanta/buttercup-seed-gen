...
# Check that task directory exists (from builder-default.sh)
if [ ! -d "$TASK_DIR" ]; then
    echo "[builder-codequery] ERROR: Task directory $TASK_DIR does not exist"
    echo "[builder-codequery] Run builder-default.sh first"
    exit 1
fi
...
LANG="c"
PROJECT_YAML="$TASK_DIR/fuzz-tooling/oss-crs/projects/$PROJECT_NAME/project.yaml"
